...
The next guardrail is result explanation. `app/studyBuilder/studyRunExplanation.js` turns a durable `study_runs` ledger record into a deterministic explanation seed. The seed records the run id, study, subject, route, effective date window, summary items, evidence links, snapshot references, confidence, and caveats. The generated contract lives at `docs/study-run-explanation-contract.json`, checked by `node scripts/export_study_run_explanation_contract.mjs --check`.

This is the same discipline in a new place. A future assistant should not look at a chart and improvise. It should ask: what run actually completed, what dates did the ledger record, what summary metrics were saved, were there warnings, did the requested and actual windows differ, and were any annualized metrics shown on a short window? If the run failed, the explanation seed blocks result conclusions entirely. The assistant may explain the failure, but it cannot pretend there was a valid investment result.
