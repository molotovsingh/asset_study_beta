...
- explanation seeds use `GET /api/study-runs` ledger payloads; failed runs, clipped windows, missing summaries, missing evidence references, blocked proxy TRI source policies, and sub-1-year annualized metric signals become explicit issues/caveats
...
- the handoff payload includes the explanation seed, replay StudyPlan, source-policy instructions, readiness flags, issues, and consumer instructions; Run History loads the backend-owned payload when available, renders it as `Assistant Handoff JSON`, and exposes the same payload through a `Download Handoff JSON` action
...
- the brief is a permission envelope for future generated prose: it says whether result conclusions are allowed, which source policy was recorded, which caveats must be mentioned, which assistant actions are allowed, and which claims are prohibited; Run History renders this as `Assistant Explanation Brief`
...
- the Python and frontend bundle consumers verify every required nested contract version in `GET /api/assistant/contract-bundle`, not only the wrapper version
